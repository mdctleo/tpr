...
MODEL_PATH='model_save_lanl.pkl'
CUTOFF = 0.5478
DEVICE = 'cpu'#'cuda'
...
def main(k_num=5):
    ...
    data = lanl.load_partial_lanl(start=150885, end=5011199, delta=10800, is_test=True)
    
    enc = GCN_local(64, 32, device=DEVICE).to(DEVICE)
    gcn_state = {}
    for k, v in states['states'][0].items():
        gcn_state[k[7:]] = v  # 'module.xxx' -> 'xxx'
    enc.load_state_dict(gcn_state)
    enc.eval()
    rnn=EmptyModel(None,None,None)
    ...
    score_stats('TITLE', preds, labels, cnts, cutoff=0.5478, ctime=None)
    #assert 0
    #'''
    # With attack
    all_mals = []
