...
def slice_data(data, t):
    ...
    ews_t = [data.ews[t]]
    # cnt_t = [data.cnt[t]]
    new_data = TData(eis_t, xs_t, ys_t, masks_t, ews=ews_t, node_map=None)
    #print (data)
    #print (new_data)
    return new_data


def evasion_attack(enc,  rnn_h0, data, tgt_t, tgt_u, tgt_v, CUTOFF, K=50, N_choice=10000):
    # enc.train()
    ...
    for num_edges in tqdm(range(K)):
        ...
        trial_enc_out, _,raw_adj = enc.forward(data, mask_enum=2, h0=rnn_h0 ,extra_edges=extra_edges)
        
        pred = enc.decode([tgt_u], [tgt_v], trial_enc_out[tgt_t])
        print (tgt_t,tgt_u,tgt_v,pred,'added:%d'%num_edges)
        #raw_adj.retain_grad()
        pred.backward()
        # print(raw_adj.is_leaf)
        # print(raw_adj.shape)
        # print(raw_adj.grad.sum())
        ...
        for num_index, index in enumerate(torch.topk(all_grad.view(-1), N_choice, largest=False).indices):
            ...
            if raw_adj[new_t][new_u,new_v] == 0 and (new_u,new_v) not in extra_edges[new_t]: # test - not already exist
                new_score = enc.decode([new_u],[new_v],trial_enc_out[new_t]).item()
