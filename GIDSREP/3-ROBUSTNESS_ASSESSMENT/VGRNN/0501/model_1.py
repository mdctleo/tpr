...
class VGRNN(nn.Module):
    ...
    '''
    def forward(self, data, mask_enum, h0=None,extra_edges=None):
        zs = []
        ...
        for i in range(data.T):
            ...
            if extra is not None and len(extra) > 0:
                ei = torch.cat( (ei, torch.LongTensor(extra).T), 1)
                ew = torch.cat( (ew, torch.ones(len(extra))*ew.mean()), 0)
            raw_adj, normed_adj = self.gcn_norm_adj(ei, ew, len(xs))
            # Doesn't use weighted edges

            h,z = self.forward_once(xs, normed_adj, h)
            zs.append(z)
            adjs.append(raw_adj)

        return torch.stack(zs), h,adjs
    ...
    def gcn_norm_adj(self, ei, ew, N_node, need_grad=True):
        import time
        raw_adj = torch.eye(N_node)
        raw_adj[ei[0],ei[1]] = ew
        if need_grad:
            raw_adj.requires_grad_()  # TODO: hard-coded here for gradient propagation!
        raw_adj.requires_grad_()
