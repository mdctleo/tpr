...
MODEL_PATH='model_save_LANL_ARGUS.pkl'
CUTOFF = 0.7746
DEVICE = 'cuda'
...
class Argus_LANL(nn.Module):
    def __init__(self,  h_dim, z_dim, device):
        super(Argus_LANL, self).__init__()
        self.device = device
        self.c1 = GCNConv(15611, h_dim, add_self_loops=True)
        self.relu = nn.ReLU()
    ...
    def forward_once(self, data, mask_enum, i, ret_adj=False, extra=None):
        ...
        ea = data.ea_masked(mask_enum, i).to(self.device)
        ...
        if extra is not None and len(extra) > 0:
            ...
            ea = torch.cat((ea,torch.zeros((len(extra),10),device=self.device)),0)
        raw_adj, normed_adj = self.gcn_norm_adj(ei, ew, len(x), need_grad=ret_adj)
        x1 = self.c1(x, normed_adj)
        x = self.c2(x1, normed_adj)
        x = self.relu(x)
        x = self.drop(x)
...
def main():
    ...
    data = lanl.load_partial_lanl(start=150885, end=1270000, delta=3600, is_test=True,use_flows=True)

    enc = Argus_LANL(32, 32, device=DEVICE).to(DEVICE)
    gcn_state = {}
    ...
    for tgt_t,tgt_u,tgt_v in all_mals:
        ...
        rnn.train()
        all_extra_edges = evasion_attack(enc, rnn, rnn_h0, data_t, 0, tgt_u, tgt_v, CUTOFF, K=max(ALL_KS), N_choice=10000)
        print(len(all_extra_edges))
