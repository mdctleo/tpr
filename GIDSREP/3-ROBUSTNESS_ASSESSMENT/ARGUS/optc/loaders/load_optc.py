...
'''
def load_partial_lanl(start=140000, end=156659, delta=8640, is_test=False, use_flows=False, ew_fn=std_edge_w, ea_fn=std_edge_a):
    ...
    while keep_reading:
        while line:
            ...
            if ts >= next_split:
                ...
                #print(len(edges_t))
                # if len(edges_t)==2:
                #       print(torch.tensor(list(zip(*edges_t.values()))[1]))
                #       print(list(zip(*edges_t.keys())))
                if len(edges_t):
