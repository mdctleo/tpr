...
from models.argus import DropEdge
import loaders.load_optc as lanl
from tqdm import tqdm
from attack_lib import evasion_attack
...
MODEL_PATH='model_save_OPTC_ARGUS.pkl'
CUTOFF = 0.9919#0.9951
DEVICE = 'cpu'#'cuda'
...
class Argus_OPTC(nn.Module):
    ...
    def __init__(self, h_dim, z_dim, device='cpu'):
        super(Argus_OPTC, self).__init__()
        self.device = device
        self.de = DropEdge(0.5)
        self.c1 = GCNConv(815, h_dim, add_self_loops=True)
    ...
    def inner_forward(self, data, mask_enum, ret_adj=False, extra_edges=None):
        ...
        for i in range(data.T):
            ...
            if ret_adj:
                ...
                # if torch.isnan(self.forward_once(data, mask_enum, i, ret_adj=ret_adj, extra=extra)).any():
                #     print(i)
    ...
    def forward_once(self, data, mask_enum, i, ret_adj=False, extra=None):
        if data.dynamic_feats:
            x = data.xs[i].to(self.device)
        else:
            x = data.xs.to(self.device)
        ei = data.ei_masked(mask_enum, i).to(self.device)
        ew = data.ew_masked(mask_enum, i).to(self.device)
        ...
        x = self.c3(x, normed_adj)
        x = self.relu(x)
        x = self.drop(x)
        #x = self.c4(x, ei, edge_weight=ew)
        x = self.c4(x, normed_adj)
        if ret_adj:
            return self.ac(x), raw_adj
        else:
            return self.ac(x)
...
def main(k_num=5):
    ...
    data = lanl.load_partial_lanl(start=573290, end=745983, delta=360, is_test=True)
    # for i in range(data.T):
    #     if all(element == 0.5 for element in data.ews[i]):
    #         print(i)
    #data =pickle.load(open('optc_data.pickle','rb'))
    #print(data.ews[294])
    #assert 0

    enc = Argus_OPTC(64, 64, device=DEVICE).to(DEVICE)
    gcn_state = {}
    for k, v in states['states'][1].items():
        if k[5]=='m':
          gcn_state[k[12:]] = v  # 'module.xxx' -> 'xxx'
    enc.load_state_dict(gcn_state)
    enc.eval()
    for p in enc.parameters():
        p.requires_grad = False
    rnn = GRU(64, 64, 32).to(DEVICE)
    rnn_state = {}
    for k, v in states['states'][1].items():
        if k[0]=='r':
    ...
    # for index,t in enumerate(range(data.T)):
    #     #print(index)
    #     if len(data.eis[t][0])==1:
    #         print(index)
    ...
    for t in tqdm(range(data.T)):
        pred = enc.decode(data.eis[t][0], data.eis[t][1], trial_rnn_out[t])
        #print(pred)
        preds.append(pred.cpu())
        labels.append(data.ys[t].cpu())
        cnts.append(data.cnt[t].cpu())
    ...
    for tgt_t,tgt_u,tgt_v in all_mals:
        ...
        #tgt_t, tgt_u, tgt_v=357,804,810
        i+=1

        rnn.eval()
        enc_out, raw_adj = enc.forward(data, mask_enum=2, no_grad=False, ret_adj=True)
        rnn_out = rnn.forward(enc_out, rnn_h0)
        init_pred = enc.decode([tgt_u], [tgt_v], rnn_out[tgt_t]).item()

        #extra_edges = evasion_attack(enc, rnn, rnn_h0, data, tgt_t, tgt_u, tgt_v, CUTOFF, K=5, N_choice=10)
        extra_edges,aa = evasion_attack(enc, rnn, rnn_h0, data, tgt_t, tgt_u, tgt_v, CUTOFF, K=k_num, N_choice=100000)

        rnn.eval()
        print(aa)
        enc_out, raw_adj = enc.forward(data, mask_enum=2, no_grad=False, ret_adj=True, extra_edges=aa)
        rnn_out = rnn.forward(enc_out, rnn_h0)
        final_pred = enc.decode([tgt_u], [tgt_v], rnn_out[tgt_t]).item()
        cover_preds = []
        for prev_t, es in aa.items():
        ...
        all_cvrs.append(cover_preds)
    print ("Avg init: %.6f; stealthy: %.6f"%(np.mean(all_init), np.mean([p>CUTOFF for p in all_init])))
    print ("Avg final: %.6f; stealthy: %.6f"%(np.mean(all_final), np.mean([p>CUTOFF for p in all_final])))
    flatten_cvrs = [v for cvr in all_cvrs for v in cvr]
    print ("Avg cvrs: %.6f; stealthy: %.6f"%(np.mean(flatten_cvrs), np.mean([p>CUTOFF for p in flatten_cvrs])))
    final_succeed = []
    for i in range(len(all_final[k])):
        is_succeed = (all_final[i] > CUTOFF)
        for cvr_pred in all_cvrs[i]:
            if not is_succeed:
                break
            is_succeed = is_succeed and (cvr_pred > CUTOFF)
        final_succeed.append(is_succeed)
    print ("Final atk succeed: %.4f"%(np.mean(final_succeed)))
    #'''

if __name__ == '__main__':
    main(k_num=10)#k_num为逃逸边数量