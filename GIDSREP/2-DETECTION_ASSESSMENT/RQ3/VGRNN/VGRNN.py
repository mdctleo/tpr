...
import loaders.load_n9000 as ln 
import loaders.load_oilrig as lo 
import loaders.load_sandworm as ls 
import loaders.load_wiazrd as lw
...
if __name__ == '__main__':
    ...
    print(PATIENCE)
    DELTA=  int(60*arg.snapshot*1000)
    print(DELTA)
    if arg.dataset=='oilrig':
        TR_START=0
        TR_END=lo.DATE_OF_EVIL_LANL-max((lo.DATE_OF_EVIL_LANL - TR_START) // 20, DELTA*2)
        VAL_START=TR_END
        VAL_END=lo.DATE_OF_EVIL_LANL

        TE_START=lo.DATE_OF_EVIL_LANL
        TE_END = lo.TIMES['all']

        LOADER = lo.load_lanl_dist 
    elif arg.dataset=='sandworm':
        TR_START=0
        TR_END=ls.DATE_OF_EVIL_LANL-max((ls.DATE_OF_EVIL_LANL - TR_START) // 20, DELTA*2)
        VAL_START=TR_END
        VAL_END=ls.DATE_OF_EVIL_LANL

        TE_START=ls.DATE_OF_EVIL_LANL
        TE_END = ls.TIMES['all']

        LOADER = ls.load_lanl_dist 
    elif arg.dataset=='wizard':
        TR_START=0
        TR_END=lw.DATE_OF_EVIL_LANL-max((lw.DATE_OF_EVIL_LANL - TR_START) // 20, DELTA*2)
        VAL_START=TR_END
        VAL_END=lw.DATE_OF_EVIL_LANL

        TE_START=lw.DATE_OF_EVIL_LANL
        TE_END = lw.TIMES['all']

        LOADER = lw.load_lanl_dist 
    elif arg.dataset=='n9000':
        TR_START=0
        TR_END=ln.DATE_OF_EVIL_LANL-max((ln.DATE_OF_EVIL_LANL - TR_START) // 20, DELTA*2)
        VAL_START=TR_END
        VAL_END=ln.DATE_OF_EVIL_LANL

        TE_START=ln.DATE_OF_EVIL_LANL
        TE_END = ln.TIMES['all']

        LOADER = lo.load_lanl_dist 
    else:
        raise NotImplementedError('Only the 0501 data set is supported in this release')
    OUT_F = 'result_RQ3.txt'
    ...
    with open(OUT_F, 'a+') as f:
        ...
        f.write("Delta: %0.3f" % (DELTA/3600)+'h\n')
