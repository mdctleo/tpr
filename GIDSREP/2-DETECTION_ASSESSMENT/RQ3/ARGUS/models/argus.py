...
class Argus_LANL(GCN):
    def __init__(self, data_load, data_kws, h_dim, z_dim, device):
        ...
        self.ac = nn.Tanh()
        self.c3 = GCNConv(h_dim, z_dim, add_self_loops=True)
        nn4 = nn.Sequential(nn.Linear(6, 8), nn.ReLU(), # lanl:  10; 0501: 6; cic: 6
...
class DetectorEncoder(Euler_Embed_Unit):
    def __init__(self, module: Euler_Embed_Unit, device, dataset, **kwargs):
        ...
        if dataset == 'O_n9000':
            self.num_nodes = 8745
            self.ap_loss = APLoss(pos_len=1858461, margin=0.8, gamma=0.1, surrogate_loss='squared', device=device) 
        elif dataset == 'L_n9009':
            self.num_nodes = 8745
            self.ap_loss = APLoss(pos_len=1858461, margin=0.8, gamma=0.1, surrogate_loss='squared', device=device)
        elif dataset == 'O_oilrig':
            self.num_nodes = 17503
            self.ap_loss = APLoss(pos_len=3790748, margin=0.8, gamma=0.1, surrogate_loss='squared', device=device)
        elif dataset == 'L_oilrig':
            self.num_nodes = 17503
            self.ap_loss = APLoss(pos_len=3790748, margin=0.8, gamma=0.1, surrogate_loss='squared', device=device)
        elif dataset == 'O_sandworm':
            self.num_nodes = 17503
            self.ap_loss = APLoss(pos_len=3802433, margin=0.8, gamma=0.1, surrogate_loss='squared', device=device)
        elif dataset == 'L_sandworm':
            self.num_nodes = 17503
            self.ap_loss = APLoss(pos_len=3802433, margin=0.8, gamma=0.1, surrogate_loss='squared', device=device)
        elif dataset == 'O_wizard':
            self.num_nodes = 17503
            self.ap_loss = APLoss(pos_len=3807667, margin=0.8, gamma=0.1, surrogate_loss='squared', device=device)
        elif dataset == 'L_wizard':
            self.num_nodes = 17503
            self.ap_loss = APLoss(pos_len=3807667, margin=0.8, gamma=0.1, surrogate_loss='squared', device=device)
