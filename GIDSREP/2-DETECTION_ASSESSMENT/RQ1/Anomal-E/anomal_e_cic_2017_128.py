...
# Dividing line between train_data and test_data
split=559918
file_name = "cic_2017_1.csv"
ndim_out = 128
...
data['Source Port'] = data["Source Port"].apply(str)
data['Destination IP'] = data["Destination IP"].apply(str)
data['Destination Port'] = data["Destination Port"].apply(str)


data.drop(columns=["Source Port", "Destination Port",'Timestamp','Label','time_int','Flow ID'], inplace=True)
...
encoder = ce.TargetEncoder(cols=['Protocol','Fwd PSH Flags','Bwd PSH Flags', 'Fwd URG Flags', 'Bwd URG Flags'])
...
for epoch in range(epochs):
    ...
    if loss < best:
        ...
        best_t = epoch
        cnt_wait = 0
        torch.save(dgi.state_dict(), 'best_dgi_64.pkl')
    else:
        cnt_wait += 1
...
dgi.load_state_dict(torch.load('best_dgi_64.pkl'))
