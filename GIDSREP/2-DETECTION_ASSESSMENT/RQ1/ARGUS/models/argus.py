...
class DetectorEncoder(Euler_Embed_Unit):
    def __init__(self, module: Euler_Embed_Unit, device, dataset, **kwargs):
        ...
        if dataset == 'OPTC':
            self.num_nodes = 815#
            self.ap_loss = APLoss(pos_len=2830742, margin=0.8, gamma=0.1, surrogate_loss='squared', device=device) # optc
        elif dataset == 'LANL':
            self.num_nodes = 15611#cic2830742
            self.ap_loss = APLoss(pos_len=283573, margin=0.1, gamma=0.01, surrogate_loss='squared', device=device)
