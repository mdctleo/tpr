...
def parse_args():
    ...
    args.add_argument('-k', "--dimensions", default = 200, type=int, help="Number of Dimension")
    args.add_argument('-ip', '--input', default = 'dataset/cic/cic_5.csv', help='Dataset filename')
    ...
    args.add_argument('-w', '--trainwin', type=int, default = 98,#159#98 49 25 17
    ...
    args.add_argument('-d', '--dataset', type=str, default='lanl',
                      help='Name of the dataset')
    args.add_argument('--kde_file', type=str, default=None,
                      help='Path to precomputed KDE edge features pickle')
...
if __name__=="__main__":
    ...
    # Load KDE features if provided
    kde_dict = None
    if args.kde_file and os.path.isfile(args.kde_file):
        print(f"Loading KDE features from {args.kde_file}")
        with open(args.kde_file, 'rb') as f:
            kde_dict = pickle.load(f)
        print(f"  Loaded KDE vectors for {len(kde_dict)} edge pairs")

    ad_long_term = AnomalyDetection(args, node_list, node_map, long_term_embs, idx=0, kde_dict=kde_dict)
    param_file_name = '_' + args.dataset + '_' + file + '_d' + str(args.dimensions)
    edge_scores_df, metrics_dict = ad_long_term.anomaly_detection(graphs, param_file='weights/param' + param_file_name)

    # Save full results for post-processing
    results_data = {
    ...
    results_pkl_path = os.path.join('results', args.dataset + '_predictions.pkl')
    os.makedirs('results', exist_ok=True)
    with open(results_pkl_path, 'wb') as f:
        pickle.dump(results_data, f, protocol=pickle.HIGHEST_PROTOCOL)
    print(f"[main.py] Saved full results to {results_pkl_path}")
