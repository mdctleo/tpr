...
from loaders.load_cic import load_lanl_dist
...
DATASET_TAG = 'cic'  # Overridden per-dataset to namespace all file I/O
SCORE_FILE = 'scores.txt'

def _tmp_file():
    return f'tmp_{DATASET_TAG}.dat'

def _model_save_file():
    return f'model_save_{DATASET_TAG}.pkl'
...
# KDE/flow feature params (threaded from run_all → workers)
KDE_FILE = None
USE_FLOWS = False
KDE_DECODE = False
...
def init_procs(rank, world_size, rnn_constructor, rnn_args, worker_constructor, worker_args, 
    ...
    if rank == world_size-1:
        ...
        if just_test:
            ...
            states = pickle.load(open(_model_save_file(), 'rb'))
        ...
        for te_start,te_end in times['te_times']:
            ...
            st, raw_preds = test(model, h0, test_times, rrefs)
            for s in st:
                s['TPE'] = tpe
                s['tr_time'] = tr_time
    ...
    if rank == world_size-1:
        pickle.dump(
            {'stats': stats, 'raw_preds': all_raw_preds},
            open(_tmp_file(), 'wb+'),
...
def test(model, h0, times, rrefs):
    ...
    print("Scoring")
    scores, labels, weights, eis = model.score_all_with_edges(zs)
    stats.append(
            score_stats(
    ...
    # Build per-edge raw predictions for post-processing
    raw_preds = build_raw_predictions(scores, labels, eis, model.cutoff)
    
    # Then reset model to having all workers for future tests
    model.num_workers = len(rrefs)
    return stats, raw_preds
...
def build_raw_predictions(scores_list, labels_list, eis_list, cutoff):
    """
    Build per-edge prediction records from per-snapshot tensors.

    Returns a list of dicts, each with keys:
        src_id, dst_id, score, label, pred, snapshot
    """
    rows = []
    for snap_idx, (sc, lb, ei) in enumerate(zip(scores_list, labels_list, eis_list)):
        sc_t = sc.detach()
        lb_t = lb.clamp(max=1)
        # ei is a 2 x E tensor (src_ids, dst_ids)
        num_edges = ei.size(1)
        for e in range(num_edges):
            score_val = sc_t[e].item()
            label_val = int(lb_t[e].item())
            # EULER: low score = anomalous, so pred=1 when score <= cutoff
            pred_val = 1 if score_val <= cutoff else 0
            rows.append({
                'src_id': int(ei[0, e].item()),
                'dst_id': int(ei[1, e].item()),
                'score': score_val,
                'label': label_val,
                'pred': pred_val,
                'snapshot': snap_idx,
...
def run_all(workers, rnn_constructor, rnn_args, worker_constructor, 
            ...
            kde_file=None, use_flows=False, kde_decode=False):
    '''
    ...
    # Set module-level globals for KDE/flow features so get_work_units
    # can include them in worker kwargs (inherited by forked processes)
    global KDE_FILE, USE_FLOWS, KDE_DECODE, DATASET_TAG
    KDE_FILE = kde_file
    USE_FLOWS = use_flows
    KDE_DECODE = kde_decode
    ...
    result = pickle.load(open(_tmp_file(), 'rb'))
    os.remove(_tmp_file())  # Clean up temp file

    # Handle both old format (list) and new format (dict with stats+raw_preds)
