...
class GCN(Euler_Embed_Unit):
    ...
    def __init__(self, data_load, data_kws, h_dim, z_dim):
        ...
        # Layer 2: NNConv if edge attributes present, else GCNConv
        self.use_ea = getattr(self.data, 'ea_dim', 0) > 0
        if self.use_ea:
            ea_dim = self.data.ea_dim
            mlp_h = max(ea_dim, 8)
            nn_ea = nn.Sequential(
                nn.Linear(ea_dim, mlp_h),
                nn.ReLU(),
                nn.Linear(mlp_h, h_dim * z_dim)
            )
            self.c2 = NNConv(h_dim, z_dim, nn_ea, aggr='mean')
        else:
    ...
    def forward_once(self, mask_enum, i):
        ...
        ea = self.data.ea_masked(mask_enum, i) if self.use_ea else None

        # DropEdge: apply same mask to ei, ew, and ea
        if self.training and self.de.p > 0:
            mask = torch.rand(ei.size(1))
            keep = mask > self.de.p
            ei = ei[:, keep]
            if ew is not None:
        ...
        # Layer 1: always GCNConv
        x = self.c1(x, ei, edge_weight=ew)
        x = self.relu(x)
        x = self.drop(x)

        # Layer 2: NNConv with edge attrs if available, else GCNConv
        if self.use_ea and ea is not None:
            x = self.c2(x, ei, edge_attr=ea.t())  # NNConv expects (E, F)
        else:
