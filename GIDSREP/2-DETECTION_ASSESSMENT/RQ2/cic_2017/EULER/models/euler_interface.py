...
class Euler_Embed_Unit(nn.Module):
    ...
    def decode(self, src, dst, z, kde_vecs=None):
        base_logit = (z[src] * z[dst]).sum(dim=1)
        if kde_vecs is not None and hasattr(self, 'kde_mlp'):
            kde_adj = self.kde_mlp(kde_vecs).squeeze(1)
            return torch.sigmoid(base_logit + kde_adj)
...
class Euler_Encoder(DDP):
    ...
    def decode(self, e, z, kde_vecs=None):
        '''
        ...
        kde_vecs : torch.Tensor or None
            Optional (E, kde_dim) tensor of KDE vectors for decode-time adjustment
        '''
        src, dst = e 
        return self.module.decode(
            src, dst, z, kde_vecs=kde_vecs
        )
