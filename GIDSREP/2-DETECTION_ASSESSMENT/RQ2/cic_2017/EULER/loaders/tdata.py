...
class TData(Data):
    ...
    def __init__(self, eis, xs, ys, masks, ews=None, nmap=None, eas=None, kde_dict=None, **kwargs):
        ...
        # Edge attributes (KDE vectors in encoder mode, flow features in reduced mode)
        self.eas = eas
        if eas and len(eas) > 0:
            self.ea_dim = eas[0].size(0)  # (feat_dim, num_edges)
        else:
            self.ea_dim = 0

        # KDE dictionary for decode-time lookup (kde_decode mode)
        self.kde_dict = kde_dict
        if kde_dict and len(kde_dict) > 0:
            self.kde_dim = len(next(iter(kde_dict.values())))
        else:
            self.kde_dim = 0
    ...
    def ea_masked(self, enum, t):
        '''Returns masked edge attributes at timestep t, or None if no eas.'''
        if self.eas is None or self.ea_dim == 0:
            return None
    ...
    def get_kde_tensor(self, src_t, dst_t):
        '''
        Look up KDE vectors for a batch of (src, dst) pairs.
        Returns (E, kde_dim) float tensor, or None if no kde_dict.
        '''
        ...
        for s, d in zip(src_t.tolist(), dst_t.tolist()):
            vecs.append(self.kde_dict.get((s, d), zero))
