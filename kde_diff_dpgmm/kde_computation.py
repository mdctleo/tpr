...
Supports two modes:
1. Raw timestamps (default): KDE computed on raw timestamp values using scipy gaussian_kde
   with hand-crafted RKHS features (quadrature, moments, Fourier, quantiles).
2. Timestamp differences (--use_timestamp_diffs): Fits a truncated Dirichlet Process
   Gaussian Mixture Model (DPGMM) via Pyro SVI on inter-arrival times, then evaluates
   the learned density on a uniform grid to produce a fixed-size vector.
   GPU inference uses batched Coordinate Ascent Variational Inference (CAVI)
   with closed-form updates — no Pyro SVI workers needed.

Usage:
    # Raw timestamps mode (original)
    python kde_computation.py kairos_kde_ts CLEARSCOPE_E3
    
    # Timestamp differences mode (DPGMM)
...
def _gpu_batched_dpgmm_cavi(
    ...
    Args:
        z_diffs_list  : list of 1-D float32 np arrays (z-scored inter-arrival diffs)
    ...
    order = sorted(range(len(z_diffs_list)), key=lambda i: len(z_diffs_list[i]))
...
class KDEVectorComputer:
    """
    Computes RKHS vectors from edge timestamps.

    Supports two modes:
    1. Raw timestamps: scipy gaussian_kde with hand-crafted RKHS features
    2. Timestamp differences (use_timestamp_diffs=True):
       Fits a truncated DPGMM (Pyro SVI) on inter-arrival times, builds
       a sklearn GaussianMixture, and evaluates density on a uniform grid
       of *rkhs_dim* points.  The resulting vector is the density profile.
    """
...
def compute_rkhs_vectors(
    ...
    effective_workers = n_workers if kde_computer.use_timestamp_diffs else 1
    ...
    if effective_workers > 1:
        ...
        # ---- Phase A: prepare z_diffs (CPU, vectorised) ----
        logger.info("Phase A: computing z_diffs for all frequent edges...")
        edge_keys, z_diffs_list = [], []
        skipped = 0
        for edge, timestamps in frequent_edges:
            ts    = np.sort(np.array(timestamps, dtype=np.float64))
            diffs = np.abs(np.diff(ts)).astype(np.float32)  # absolute value of consecutive diffs
            if len(diffs) < 2:
                skipped += 1
                continue
            ...
            z_diffs_list.append((diffs - mu) / sig)
        ...
        if cuda_avail:
            ...
            with ctx.Pool(processes=effective_workers) as pool:
                ...
                for edge, vector in tqdm(
                    pool.imap_unordered(_dpgmm_edge_worker, worker_args, chunksize=cs),
