...
Both modes fit a truncated Dirichlet Process Gaussian Mixture Model (DPGMM) to
approximate the density of edge temporal observations, then evaluate the learned
density on a uniform grid to produce a fixed-size feature vector.
GPU inference uses batched Coordinate Ascent Variational Inference (CAVI) with
closed-form updates — no Pyro SVI workers needed.

Supports two modes:
1. Raw timestamps (default): DPGMM fitted on z-scored raw timestamp values.
2. Timestamp differences (--use_timestamp_diffs): DPGMM fitted on z-scored
   inter-arrival times (consecutive timestamp differences).

Usage:
    # Raw timestamps mode (default — DPGMM on raw timestamps)
    python kde_computation.py kairos_kde_ts CLEARSCOPE_E3
    
    # Timestamp differences mode (DPGMM on inter-arrival diffs)
...
# DPGMM dependencies (for timestamp diffs mode)
try:
    import pyro
...
def _dpgmm_edge_worker(packed_args: tuple):
    """
    ...
    """
    # Must be first: hides GPU before torch is imported / CUDA is touched.
    import os as _os
    ...
    try:
        ...
        sorted_ts = np.sort(timestamps_array)
        diffs = np.abs(np.diff(sorted_ts))  # absolute value of consecutive diffs
        ...
        if diff_std < 1e-12:
            diff_std = 1.0
        z_diffs = (diffs - diff_mean) / diff_std

        dpgmm = _PyroDPGMM(device="cpu")
        means, stds, weights = dpgmm.fit(z_diffs)

        gmm = _PyroDPGMM.build_gmm(means, stds, weights)
        if gmm is None:
            return edge, None

        grid_min, grid_max = float(z_diffs.min()), float(z_diffs.max())
...
def _gpu_batched_dpgmm_cavi(
    ...
    Args:
        z_data_list   : list of 1-D float32 np arrays (z-scored data: raw timestamps
                        or inter-arrival diffs depending on mode)
...
# ---------------------------------------------------------------------------
# Embedded DPGMM (ported from tpr/pyro_dpgmm.py) — used only when
# use_timestamp_diffs=True.  Priors are scaled to the observed data range
# of each edge (no external MinMax scaling required).
# ---------------------------------------------------------------------------
...
class _PyroDPGMM:
    ...
    Hardcoded defaults (not exposed in YAML):
        ...
        num_epochs     = 2000   (max SVI epochs)
    ...
    def fit_with_init(
        self,
        data_np: np.ndarray,
        init_means_np: np.ndarray,
        init_vars_np: np.ndarray,
        num_epochs: int = None,
        batch_size: int = None,
        patience: int   = None,
    ):
        """
        Fit DPGMM using pre-computed K-Means init (skips _kmeans_init).

        Called by _dpgmm_svi_worker when GPU batch K-Means has already
        produced init_means/init_vars for this edge.
        """
        num_epochs = num_epochs or self.DEFAULT_NUM_EPOCHS
        batch_size = batch_size or self.DEFAULT_BATCH_SIZE
        patience   = patience   or self.DEFAULT_PATIENCE
        ...
        data_t = torch.tensor(data_np, device=self.device, dtype=torch.float32).squeeze(-1)
        N = data_t.shape[0]
        self.K           = len(init_means_np)
        self._batch_size = min(batch_size, N)

        self._set_data_scaled_priors(data_t)
        # Inject GPU-computed init — skip sklearn K-Means entirely
        self.init_means     = torch.tensor(init_means_np, device=self.device,
                                           dtype=torch.float32)
        self.init_variances = torch.tensor(init_vars_np,  device=self.device,
...
class KDEVectorComputer:
    ...
    Both modes fit a truncated Dirichlet Process Gaussian Mixture Model (DPGMM)
    and evaluate the learned density on a uniform grid of *rkhs_dim* points.

    Modes:
    1. Raw timestamps (default): DPGMM fitted on z-scored raw timestamp values.
    2. Timestamp differences (use_timestamp_diffs=True): DPGMM fitted on z-scored
       inter-arrival times.  GPU uses batched CAVI; CPU uses Pyro SVI workers.
    """
    ...
    def timestamp_diffs_to_rkhs_vector(self, timestamp_diffs: np.ndarray) -> np.ndarray:
        ...
        using a truncated DPGMM fitted via Pyro SVI.

        Pipeline:
        1. Filter out non-positive diffs.
        2. Fit _PyroDPGMM (K=20, early-stopped SVI) on the valid diffs.
        3. Wrap the truncated (99 %) GMM params into a sklearn GaussianMixture.
        4. Evaluate the density on a uniform grid spanning
           [min(diffs), max(diffs)] with *rkhs_dim* points.
        ...
        try:
            # 1. Use absolute value of all diffs (handles any clock jitter/unsorted edge cases)
            diffs = np.abs(timestamp_diffs)
            if len(diffs) < 2:
                logger.warning("Not enough timestamp diffs, using zero vector")
                return np.zeros(self.rkhs_dim, dtype=np.float32)

            # Z-score standardise to unit scale before SVI.
            # Raw diffs are often nanosecond-scale (~1e9), which drives
            # prior parameters (lambda0, beta0) to extreme values and causes
            # NaN gradients.  Standardising keeps arithmetic in a safe range
            # while preserving the density *shape* (the only thing that
            # matters for the feature vector).
            diff_mean = float(np.mean(diffs))
            diff_std  = float(np.std(diffs))
            if diff_std < 1e-12:
                diff_std = 1.0
            ...
            # 2. Fit DPGMM on standardised diffs
            dpgmm = _PyroDPGMM()
            means, stds, weights = dpgmm.fit(z_diffs)
            ...
            # 4. Evaluate density on a uniform grid (standardised domain)
            grid_min = float(z_diffs.min())
            grid_max = float(z_diffs.max())
            if grid_max - grid_min < 1e-12:
                # Degenerate: all diffs identical -> flat density
            ...
            logger.warning(f"DPGMM computation on diffs failed: {e}, using zero vector")
            return np.zeros(self.rkhs_dim, dtype=np.float32)
...
def compute_rkhs_vectors(
    ...
    mode_str = "timestamp DIFFERENCES (DPGMM)" if kde_computer.use_timestamp_diffs else "raw timestamps (DPGMM)"
    ...
    if effective_workers > 1:
        # ----------------------------------------------------------------
        # GPU-assisted parallel pipeline (4 phases):
        #   A. CPU: vectorised z_diffs preparation (trivially parallel)
        #   B. GPU: batch K-Means over ALL edges simultaneously
        #   C. CPU: SVI workers (pre-seeded, skip sklearn K-Means)
        #   D. GPU: batch GMM density evaluation over ALL edges
        # Falls back to original CPU-only path when no CUDA is available.
        # ----------------------------------------------------------------
        cuda_avail = torch.cuda.is_available()
        gpu_dev    = "cuda" if cuda_avail else "cpu"

        # ---- Phase A: prepare z-scored data (CPU, vectorised) ----
        if kde_computer.use_timestamp_diffs:
            logger.info("Phase A: z-scoring timestamp diffs for all frequent edges...")
        else:
        ...
        for edge, timestamps in frequent_edges:
            ...
            if kde_computer.use_timestamp_diffs:
                data = np.abs(np.diff(ts)).astype(np.float32)
                if len(data) < 2:
                    skipped += 1
                    continue
            else:
        ...
        if cuda_avail:
            # ---- GPU-native batched CAVI (replaces old Phases B + C + D) ----
            # Single all-GPU pass: no Pyro param store, no CPU worker processes,
            # no IPC. CAVI converges in ~50–150 iterations vs 500 SVI epochs.
            mode_label = "timestamp diffs" if kde_computer.use_timestamp_diffs else "raw timestamps"
            logger.info(
                f"GPU CAVI: fitting DP-GMM on {len(edge_keys):,} edges ({mode_label}) "
            ...
            # No GPU: fall back to original per-edge CPU-only workers
            logger.info("No CUDA detected — falling back to CPU-only parallel workers.")
            worker_fn = _dpgmm_edge_worker if kde_computer.use_timestamp_diffs else _dpgmm_raw_ts_worker
            worker_args = [
            ...
            with ctx.Pool(processes=effective_workers) as pool:
                cs = max(1, len(worker_args) // (effective_workers * 4))
                for edge, vector in tqdm(
                    pool.imap_unordered(worker_fn, worker_args, chunksize=cs),
...
def main():
    ...
    parser.add_argument('--n_workers', type=int, default=1,
                        help='Number of parallel worker processes for DPGMM computation '
                             '(timestamp-diffs mode only). Each worker runs on CPU with '
                             'its own Pyro param store. '
                             f'Max useful value ≈ number of CPU cores '
