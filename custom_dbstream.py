...
class DBSTREAM():
    ...
    * After the connectivity graph is generated, a variant of the DBSCAN algorithm
    ...
    def __init__(
        ...
        fallback_variances: float = 1e-10
    ):
        super().__init__()
        self._time_stamp = 0
        ...
        self.fallback_variances = fallback_variances
    ...
    def _distance(point_a, point_b):
        ...
        """
        return sum((abs(point_a.get(k, 0.0) - point_b.get(k, 0.0))) ** 1 for k in {*point_a.keys(), *point_b.keys()}) ** (1 / 2)
        ...
        if len(neighbor_clusters) < 1:
            ...
            if len(self._micro_clusters) > 0:
                self._micro_clusters[max(self._micro_clusters.keys()) + 1] = DBSTREAMMicroCluster(
                    x=x, last_update=self._time_stamp, weight=1, fallback_variances=self.fallback_variances
                )
            else:
            ...
            for i in neighbor_clusters.keys():
                ...
                self._micro_clusters[i].sum_points += x['timestamp']
                self._micro_clusters[i].sum_squared_points += x['timestamp'] ** 2
                self._micro_clusters[i].count += 1

                # Incrementally update the variance
                self._micro_clusters[i].variance = (
                    (self._micro_clusters[i].sum_squared_points / self._micro_clusters[i].count)
                    - (self._micro_clusters[i].sum_points / self._micro_clusters[i].count) ** 2
                )

                # Handle zero or negative variance
                if self._micro_clusters[i].variance <= 0:
                    self._micro_clusters[i].variance = self.fallback_variances
    ...
    def _generate_clusters_from_labels(self, cluster_labels):
        # print("Generate cluster from labels called", flush=True)
        ...
        for i in range(max(cluster_labels.values()) + 1):
            ...
            # macro_cluster.variance = self._calculate_variance(macro_cluster)
            clusters[i] = macro_cluster

        #     print("Got before calculate variance with clusters: ", clusters, flush=True)

        # for cluster in clusters.values():
        #     print("Cluster variance: ", cluster.variance, flush=True)
...
class DBSTREAMMicroCluster(metaclass=ABCMeta):
    ...
    def __init__(self, x=None, last_update=None, weight=None, fallback_variances=1e-10):
        self.center = x 
        self.last_update = last_update
        self.weight = weight

        self.count = 1  # Number of points in the micro-cluster
        self.sum_points = x['timestamp']  # Sum of all points in the micro-cluster (scalar)
        self.sum_squared_points = x['timestamp']**2

        self.variance = (self.sum_squared_points / self.count) - (self.sum_points / self.count) ** 2
        self.fallback_variances = fallback_variances

        # Handle zero or negative variance
        if self.variance <= 0:
            self.variance = self.fallback_variances  # Replace with a small positive value
    ...
    def merge(self, cluster):
        ...
        self.sum_points += cluster.sum_points
        self.sum_squared_points += cluster.sum_squared_points
