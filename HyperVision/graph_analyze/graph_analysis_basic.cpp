...
auto traffic_graph::_f_exeract_feature_long2(const size_t index) const -> feature_t {
    ...
    // Append KDE density features if enabled and available
    if (kde_enabled && p_e->has_kde_density()) {
        const auto & kde_vec = p_e->get_kde_density();
        for (const auto & v : kde_vec) {
            ret.push_back(v);
        }
    } else if (kde_enabled && kde_dim > 0) {
        // Fallback: append zeros for edges without KDE data
        for (size_t i = 0; i < kde_dim; i++) {
            ret.push_back(0.0);
        }
    }
