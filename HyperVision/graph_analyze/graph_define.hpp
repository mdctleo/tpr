...
private:
    ...
    // KDE density feature support
    bool kde_enabled = false;
    size_t kde_dim = 0;
    shared_ptr<kde_feature_loader> p_kde_loader;
...
public:
    ...
    void parse_edge(void) {
        ...
        // Inject KDE density vectors into long edges if loaded
        if (kde_enabled && p_kde_loader && p_kde_loader->is_loaded()) {
            size_t kde_hit = 0;
            for (auto & p_e : *p_long_edge) {
                const auto * kde_vec = p_kde_loader->lookup(p_e->get_src_str(), p_e->get_dst_str());
                if (kde_vec) {
                    p_e->set_kde_density(*kde_vec);
                    kde_hit++;
                }
            }
            LOGF("KDE injection: %ld / %ld long edges matched.", kde_hit, p_long_edge->size());
        }
