...
void traffic_graph::config_via_json(const json & jin) {
    try {
        ...
        // KDE density feature configuration
        if (jin.count("kde_features_path")) {
            string kde_path = static_cast<string>(jin["kde_features_path"]);
            p_kde_loader = make_shared<kde_feature_loader>();
            if (p_kde_loader->load_from_csv(kde_path)) {
                kde_enabled = true;
                kde_dim = p_kde_loader->get_kde_dim();
                LOGF("KDE features enabled: dim=%ld, edges=%ld", kde_dim, p_kde_loader->size());
            } else {
                WARN("KDE features file not loaded, running without KDE.");
                kde_enabled = false;
            }
        }
